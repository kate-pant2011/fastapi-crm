...
async def get_user_list(session, roles, query):
    ...
    if query.role_name is not None and query.role_name not in ROLES:
        raise ApplicationException(
            f"Роль '{query.role_name}' не существует", 400
        )
...
async def get_user(session, user_id, roles):
    ...
    if not user:
        raise ApplicationException("Пользователь не найден, либо у Вас нет прав", 404)

    target_roles = list({role.name for role in user.roles})

    if not target_roles:
        raise ApplicationException("Роль не найдена", 404)
    ...
    if not is_admin and not is_executor:
        raise ApplicationException(
            f"Нельзя получить доступ к пользователю с ролями: {target_roles}", 403
        )
...
async def get_user_me(session, user_id):
    ...
    if not user.is_active:
        raise ApplicationException("Пользователь не архивирован", 400, {"id": user.id})
...
async def change_user(session, roles, user_id, item):
    ...
    if not user.is_active:
        raise ApplicationException(f"Пользователь '{user.name}'" , 400, {"id": user.id})
    ...
    if "branch_id" in update_data:
        ...
        if not branch:
            raise ApplicationException("Компании не найдена", 404)

        if branch.is_archived:
            raise ApplicationException(
                f"Компания с ИНН {branch.inn} архивирована", 400, {"id": branch.id}
            )
...
async def create_user(session, roles, data):
    ...
    if user is not None:
        if not user.is_active:
            raise ApplicationException("Пользователь архивирован", 400, {"id": user.id})

        raise ApplicationException(f"Email {data.email} уже используется", 400)
    ...
    if branch.is_archived:
        raise ApplicationException(f"Компания с ИНН {branch.inn} архивирована", 400, {"id": branch.id})

    if "owner" in data.roles:
        raise ApplicationException("Нельзя создать пользователя с ролью owner", 400)
...
async def restore_user(session, user_id):
    ...
    if user.is_active:
        raise ApplicationException("Пользователь уже восстановлен", 400)
...
async def resend_signup_invitation(session, user_id):
    ...
    if not user.is_active:
        raise ApplicationException(
            f"Пользователь '{user.name}' архивирован", 400, {"id": user.id}
        )
    
    if not user.must_change_password:
        raise ApplicationException(
            f"Пользователь '{user.name}' уже завершил регистрацию", 400
        )
